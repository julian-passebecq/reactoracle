...
export function useOverview() {
  return useQuery({ queryKey: ["overview"], queryFn: () => controlPlane.getOverview(), staleTime: 15_000, refetchInterval: 15_000, retry: 1 });
}

export function useWorkloads() {
  return useQuery({ queryKey: ["workloads"], queryFn: () => controlPlane.getWorkloads(), staleTime: 15_000, refetchInterval: 15_000, retry: 1 });
}

export function useInfrastructure() {
  return useQuery({ queryKey: ["infrastructure"], queryFn: () => controlPlane.getInfrastructure(), staleTime: 30_000, refetchInterval: 30_000, retry: 1 });
}

export function useMaintenance() {
  return useQuery({ queryKey: ["maintenance"], queryFn: () => controlPlane.getMaintenance(), staleTime: 30_000, refetchInterval: 30_000, retry: 1 });
}
